...
test("keeps DeepSeek credentials server-side and reports missing configuration", async () => {
  const response = await worker.fetch(new Request("https://example.test/api/generate-douyin-copy", {
    method: "POST",
...
test("allows the configured GitHub Pages origin for caption requests", async () => {
  const response = await worker.fetch(new Request("https://api.example.test/api/generate-douyin-copy", {
    method: "POST",
    headers: { "content-type": "application/json", origin: "https://frenx-lab.github.io" },
    body: JSON.stringify({ text: "这是一段用于生成抖音发布配文的卡片原文。", style: "concise" }),
  }), { ALLOWED_ORIGIN: "https://frenx-lab.github.io", ASSETS: { fetch: async () => new Response("missing", { status: 404 }) } });
...
test("returns three structured DeepSeek caption candidates", async () => {
  ...
  globalThis.fetch = async (_request, options) => {
    ...
    return Response.json({ choices: [{ message: { content: JSON.stringify({ candidates: ["版本一 #AI", "版本二 #认知", "版本三 #成长"] }) } }] });
  };
  try {
    const response = await worker.fetch(new Request("https://example.test/api/generate-douyin-copy", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ text: "这是一段用于生成抖音发布配文的卡片原文。", style: "discussion" }),
    }), { DEEPSEEK_API_KEY: "test-secret", ASSETS: { fetch: async () => new Response("missing", { status: 404 }) } });
    assert.equal(response.status, 200);
    assert.deepEqual((await response.json()).candidates, ["版本一 #AI", "版本二 #认知", "版本三 #成长"]);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
